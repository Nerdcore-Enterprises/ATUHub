    ...
    const testDriversData = [
        {
            name: "some driver",
            stars: 4,
            desc: "some information about the driver here"
        },
        {
            name: "bad driver",
            stars: 1,
                ...
                <div className='w-full flex flex-row gap-5'>
                    {/* DRIVER LIST */}
                        <div className='justify-center w-full lg:w-1/2 lg:min-w-[50%] '>
                            {
                                testDriversData.map((data, index) => {
                                    if (data.name.toLowerCase().includes(searchQuery.toLowerCase()))
                                        return(
                                            <DriverWidget key={index} driverData={data} onClick={() => onDriverClick(index)}/>
                                        );
                                    return (<></>);
                                })
                            }
                        ...
                        <>
                            <div className='grow overflow-auto pb-10'>
                                <p className="text-3xl font-semibold mb-3">
                                    {testDriversData[driverIndex].name}
                                </p>
                                <p className='lg:max-h-[50rem] lg:min-h-[50rem]'>{testDriversData[driverIndex].desc}</p>
                            </div>
                            <hr className='mb-4'></hr>
                            <div className='h-fit py-2'>
                                {testDriversData[driverIndex].applyExternally &&
                                    <a href={testDriversData[driverIndex].link}>
